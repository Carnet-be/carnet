  ...
  return (
    <div className="flex flex-wrap gap-3 items-end">
      ...
      </Autocomplete>
      {
        ...
        (searchParams.get("body") || searchParams.get("brand") || searchParams.get("model")) ? (
          <Button
...
export const CarsSections = () => {
  ...
  const { data, fetchNextPage, isFetchingNextPage, hasNextPage } =
    api.car.getCars.useInfiniteQuery(
      {
        filter: {
          body: searchParams.get("body") ? parseInt(searchParams.get("body") ?? "0") : undefined,
          brand: searchParams.get("brand") ? parseInt(searchParams.get("brand") ?? "0") : undefined,
          model: searchParams.get("model") ? parseInt(searchParams.get("model") ?? "0") : undefined,
  ...
  return (
    <div className="flex w-full flex-col items-center gap-2">
      ...
      {data?.pages.map((e) => e.result).reduce((b, a) => {
        return b.concat(a)
      }, []).length == 0 && (
          <span className="py-5 opacity-60">No cars found, try another search</span>
