...
import { type InferInsertModel, and, eq, getTableColumns, sql } from "drizzle-orm";
...
const getCars = protectedProcedure.query(async({ ctx }) => {
    ...
    const result=await  db
        .select({
            ...
            images: sql<CarSpecs[]>`IF(COUNT(${carAssets.id}) = 0, JSON_ARRAY(), json_arrayagg(json_object('id',${carAssets.id},'key',${carAssets.key})))`,
            ...
            .select(
                {
                    ...
                    body: body,
                    color: colors,
                    options: sql<CarOption[]>`IF( COUNT(${carOption.id}) = 0, JSON_ARRAY(),json_arrayagg(json_object('id',${carOption.id},'name',${carOption.name})))`,
                }
            )
            .from(cars)
            .leftJoin(carAssets, eq(cars.id, carAssets.carId))
