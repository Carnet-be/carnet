...
const filterSchema = z.object({
    ...
    status: z.enum(["pending", "published", "paused", "finished", "completed", "sold"]).optional().nullable(),
...
export const boCarsRouter = createTRPCRouter({
    getCounts: protectedProcedure.query(async ({ ctx }) => {
        return ctx.db.transaction(async (trx) => {
            const [direct] = await trx.select({ count: sql<number>`count(*)` }).from(cars).where(eq(cars.type, "direct"))
            const [auction] = await trx.select({ count: sql<number>`count(*)` }).from(cars).where(eq(cars.type, "auction"))
            return {
    ...
    getFilters: protectedProcedure.query(async ({ ctx }) => {
        return ctx.db.transaction(async (trx) => {
            const brandsData = await trx.select({ id: brand.id, label: brand.name }).from(brand)
            const modelsData = await trx.select({ id: model.id, label: model.name, brandId: model.brandId }).from(model)
            return {
                brands: brandsData,
                models: modelsData
            }
    ...
    getStatusCounts: protectedProcedure.input(filterSchema.omit({ status: true })).query(async ({ ctx, input }) => {
        ...
        const result = await ctx.db.select({ status: cars.status, count: sql<number>`count(*)` }).from(cars).where(where).groupBy(cars.status)
    ...
    getCars: protectedProcedure.input(filterSchema).query(async ({ ctx, input }) => {
        ...
        const result = await ctx.db.select({
            ...
            images: sql<{ id: number, k: string }[]>`IF(COUNT(${carAssets.id}) = 0, JSON_ARRAY(), json_arrayagg(json_object('id',${carAssets.id},'key',${carAssets.key})))`
        }).from(cars).leftJoin(carAssets, eq(cars.id, carAssets.carId)).where(where).groupBy(cars.id)
        return result
    }
    ),
});
