...
//  const {data:user} =  useAuthUser(["user"], auth);
  ...
  return  <Link href={"/auth/login"} className="rounded-lg py-3 bg-white text-black font-bold text-center">
...
const ProfileButton = () => {
  ...
  console.log(user)
  return <button onClick={()=>router.push("/dashboard")} className="btn btn-warning">{user.user?.email}</button>
};
const Overlay = () => {
  return (
